...
MODULE Residual_Jacobian
    ...
    SAVE
    PRIVATE
    PUBLIC :: get_residual_jacobian !, get_residual, get_jacobian
    ...
    SUBROUTINE get_residual_jacobian(X, P_holder, Sw_holder, Tgeo, K, &
                            cell_connections, N_cell_connections,N_cells, N_steps, Property_Dict, Well_Loc, Prev_t_step, JAC, RES)
        ...
        REAL(KIND=8), DIMENSION(N_cells*2, N_cells*2), INTENT(OUT) :: JAC

        ! Local variables
        REAL(KIND=8), DIMENSION(N_cells, N_cells) :: Jpo, Jpw, Jso, Jsw
...
!
        ...
        DO i = 1, N_cell_connections
            ...
            Jpo(c_1, c_1) = Jpo(c_1, c_1) + dFdp_o(i, 1)
            Jpo(c_1, c_2) =  dFdp_o(i, 2)
            Jpo(c_2, c_1) = -dFdp_o(i, 1)
            Jpo(c_2, c_2) = Jpo(c_2, c_2) - dFdp_o(i, 2)
            ! Locating Water Phase Flux in Jacobian w.r.t p : 2/4 
            Jpw(c_1, c_1) = Jpw(c_1, c_1) + dFdp_w(i, 1)
            Jpw(c_1, c_2) =  dFdp_w(i, 2)
            Jpw(c_2, c_1) = -dFdp_w(i, 1)
            Jpw(c_2, c_2) = Jpw(c_2, c_2) - dFdp_w(i, 2)
            !Locating Oil Phase Flux in Jacobian w.r.t s : 3/4 
            Jso(c_1, c_1) = Jso(c_1, c_1) + dFds_o(i, 1)
            Jso(c_1, c_2) =  dFds_o(i, 2)
            Jso(c_2, c_1) = -dFds_o(i, 1)
            Jso(c_2, c_2) = Jso(c_2, c_2) - dFds_o(i, 2)
            !Locating Water Phase Flux in Jacobian w.r.t s : 4/4  
            Jsw(c_1, c_1) = Jsw(c_1, c_1) + dFds_w(i, 1)
            Jsw(c_1, c_2) =  dFds_w(i, 2)
            Jsw(c_2, c_1) = -dFds_w(i, 1)
            Jsw(c_2, c_2) = Jsw(c_2, c_2) - dFds_w(i, 2)
        END DO

        DO i = 1, N_cells
            ! Oil Phase Full Jacobian w.r.t p : 1/4
            Jpo(i, i) = Jpo(i, i) - dA_odp(i) - dQ_odp(i)
            ! Water Phase Full Jacobian w.r.t p : 2/4 
            Jpw(i, i) = Jpw(i, i) - dA_wdp(i) - dQ_wdp(i)
            ! Oil Phase Full Jacobian w.r.t s : 3/4 
            Jso(i, i) = Jso(i, i) - dA_ods(i) - dQ_ods(i)
            ! Water Phase Full Jacobian w.r.t s : 4/4  
            Jsw(i, i) = Jsw(i, i) - dA_wds(i) - dQ_wds(i)
        END DO
!
        JAC(:N_cells, :N_cells)     = Jpo
        JAC(N_cells+1:, :N_cells)   = Jpw
        JAC(:N_cells, N_cells+1:)   = Jso
        JAC(N_cells+1:, N_cells+1:) = Jsw

    END SUBROUTINE get_residual_jacobian

END MODULE Residual_Jacobian
