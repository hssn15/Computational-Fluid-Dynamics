...
Swi = 0.2
Soi = 0.8
P_hist = torch.zeros(steps, 15*15).to(device = device)
P_hist[0] = torch.ones(15*15, device=device)*Pi
Sw_hist = torch.zeros(steps, 15*15).to(device = device)
Sw_hist[0] = torch.ones(15*15, device=device)*Swi

Flux_holder = torch.zeros((N, N)).to(device=device)


Residual = Residual(Dict, Well_Loc[1], Well_Loc[0], cell_connections)


X = torch.tensor([3000.0 for i in range(15)]*15 + [0.2 for i in range(15)]*15, dtype=torch.float).to(device=device)
...
for n in range(0, steps-1):
    ...
    while sum(e>tol) != 0 and iter <= CondIter:
        t0 = time.time()
        R = Residual.calculate_Residual(X, C, P_hist, Sw_hist, n, Flux_holder)
        t1 = time.time()
        JR = jacrev(Residual.calculate_Residual)(X, C, P_hist, Sw_hist, n, Flux_holder)
        t2 = time.time()
        #X0 = X0 - torch.tensor(sp.sparse.linalg.bicgstab(csc_matrix(J.cpu().numpy()), F_X0.cpu().numpy())[0], dtype=torch.float).clone().detach().to(device = device)
        #X0 = X0.cpu() - np.matmul(np.linalg.inv(J.cpu().detach().numpy()), F_X0.detach().cpu().numpy())
        #X0 = torch.tensor(X0, dtype=torch.float).to(device = device)
